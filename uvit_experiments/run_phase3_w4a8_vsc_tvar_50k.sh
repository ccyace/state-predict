#!/usr/bin/env bash
# Phase 3: U-ViT W4A8 vsc_tvar 50k sampling + FID.
set -euo pipefail
...
PHASE2="uvit_experiments/outputs/phase2_w4a8"
OUT="uvit_experiments/outputs/phase3_w4a8_vsc_tvar_50k"
LOGDIR="${OUT}/logs"
mkdir -p "${LOGDIR}"
...
if [ ! -f "${DT_CKPT}" ] || [ ! -f "${MEAN_CKPT}" ] || [ ! -f "${VSC_STATS}" ]; then
  echo "Phase 2 W4A8 artifacts missing. Run uvit_experiments/run_phase2_uvit_w4a8.sh first."
  exit 1
fi

echo "=== [1/2] sample 50k vsc_tvar (W4A8) ==="
