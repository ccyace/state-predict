#!/usr/bin/env bash
# Phase 2: U-ViT W8A8 dt correction + mean residual + tvar stats.
set -euo pipefail
...
OUT="uvit_experiments/outputs/phase2_w8a8"
LOGDIR="${OUT}/logs"
mkdir -p "${LOGDIR}"
...
echo "=== [4/7] collect dt-only closed-loop traj (mean training) ==="
if [ -f "${TRAJ_DT}" ]; then
