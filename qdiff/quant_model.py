...
class QuantModel(nn.Module):
    ...
    def set_first_last_layer_to_8bit(self):
        """Match BRECQ/QuEST LDM ImageNet PTQ: keep input/output (and time embed) at 8-bit."""
        module_list = [m for m in self.model.modules() if isinstance(m, QuantModule)]
        if not module_list:
            return
        targets = {module_list[0], module_list[-1]}
        for name, m in self.named_modules():
            if isinstance(m, QuantModule) and (
                name.endswith("input_blocks.0.0") or name.endswith("out.2")
                or name.endswith("time_embed.0") or name.endswith("time_embed.2")
            ):
                targets.add(m)
        for m in targets:
            m.weight_quantizer.bitwidth_refactor(8)
        print(
            f"set_first_last_layer_to_8bit: {len(targets)} weight quantizer(s) -> 8-bit",
            flush=True,
        )

    def disable_network_output_quantization(self):
        """Disable act quant on the last QuantModule (UNet out.2); official LDM ckpts omit it."""
        module_list = [m for m in self.model.modules() if isinstance(m, QuantModule)]
        if not module_list:
            return
        module_list[-1].disable_act_quant = True
        print("disable_network_output_quantization: last QuantModule act quant off", flush=True)
