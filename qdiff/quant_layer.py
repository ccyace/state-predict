...
class QuantModule(nn.Module):
    ...
    def __init__(self, org_module: Union[nn.Conv2d, nn.Linear, nn.Conv1d], weight_quant_params: dict = {},
        ...
        # ODE pre-quant output scale: h_out <- h_out / s (empty tensor = disabled)
        self.register_buffer("ode_output_inv_scale", torch.zeros(0))
        # DilateQuant-style: h_in <- h_in / s before matmul (paired with W[:,c]*=s_c)
        self.register_buffer("ode_input_inv_scale", torch.zeros(0))
        # Optional differentiable /s for joint (s, A_q) optimization (see joint_sa_opt.py)
        self._ode_input_inv_scale_live = None

        self.extra_repr = org_module.extra_repr

    def set_ode_output_inv_scale(self, inv_scale: torch.Tensor) -> None:
        """Set per-output-channel multiplier (1/s) applied after conv output."""
        inv = inv_scale.detach().float().flatten()
        self.ode_output_inv_scale = inv.to(device=self.weight.device)

    def set_ode_input_inv_scale(self, inv_scale: torch.Tensor) -> None:
        """Set per-input-channel multiplier (1/s) applied before conv/linear matmul."""
        inv = inv_scale.detach().float().flatten()
        self.ode_input_inv_scale = inv.to(device=self.weight.device)

    def _apply_ode_input_scale(self, inp: torch.Tensor) -> torch.Tensor:
        if self._ode_input_inv_scale_live is not None:
            inv_src = self._ode_input_inv_scale_live
        elif self.ode_input_inv_scale.numel() == 0:
            return inp
        else:
            inv_src = self.ode_input_inv_scale
        c = min(inv_src.numel(), inp.shape[1])
        if c == 0:
            return inp
        inv = inv_src[:c].to(device=inp.device, dtype=inp.dtype).view(1, c, 1, 1)
        if c == inp.shape[1]:
            return inp * inv
        return torch.cat([inp[:, :c, :, :] * inv, inp[:, c:, :, :]], dim=1)

    def _apply_ode_output_scale(self, out: torch.Tensor) -> torch.Tensor:
        if self.ode_output_inv_scale.numel() == 0:
            return out
        c = min(self.ode_output_inv_scale.numel(), out.shape[1])
        if c == 0:
            return out
        inv = self.ode_output_inv_scale[:c].to(device=out.device, dtype=out.dtype).view(1, c, 1, 1)
        if c == out.shape[1]:
            return out * inv
        return torch.cat([out[:, :c, :, :] * inv, out[:, c:, :, :]], dim=1)
    ...
    def forward(self, input: torch.Tensor, split: int = 0):
        ...
        input = self._apply_ode_input_scale(input)
        ...
        out = self._apply_ode_output_scale(out)
