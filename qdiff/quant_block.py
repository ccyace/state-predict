...
class QuantResnetBlock(BaseQuantBlock):
    def __init__(
        ...
        self.register_buffer("ode_in_inv_scale", th.zeros(0))
        self.register_buffer("ode_shortcut_inv_scale", th.zeros(0))
        self._ode_shortcut_inv_scale_live = None

    def set_ode_in_inv_scale(self, inv_scale: th.Tensor) -> None:
        """Per-input-channel multiplier (1/s_in) at block entry."""
        inv = inv_scale.detach().float().flatten()
        ref = self.conv1.weight
        self.ode_in_inv_scale = inv.to(device=ref.device)

    def set_ode_shortcut_inv_scale(self, inv_scale: th.Tensor) -> None:
        """Per-channel /s applied only on shortcut branch input (nin / conv_shortcut)."""
        inv = inv_scale.detach().float().flatten()
        ref = self.conv1.weight
        self.ode_shortcut_inv_scale = inv.to(device=ref.device)

    def _apply_ode_shortcut_in_scale(self, x: th.Tensor) -> th.Tensor:
        if self._ode_shortcut_inv_scale_live is not None:
            inv_src = self._ode_shortcut_inv_scale_live
        elif self.ode_shortcut_inv_scale.numel() == 0:
            return x
        else:
            inv_src = self.ode_shortcut_inv_scale
        c = min(inv_src.numel(), x.shape[1])
        if c == 0:
            return x
        inv = inv_src[:c].to(device=x.device, dtype=x.dtype).view(1, c, 1, 1)
        if c == x.shape[1]:
            return x * inv
        return th.cat([x[:, :c, :, :] * inv, x[:, c:, :, :]], dim=1)

    def _apply_ode_in_scale(self, x: th.Tensor) -> th.Tensor:
        if self.ode_in_inv_scale.numel() == 0:
            return x
        c = min(self.ode_in_inv_scale.numel(), x.shape[1])
        if c == 0:
            return x
        inv = self.ode_in_inv_scale[:c].to(device=x.device, dtype=x.dtype).view(1, c, 1, 1)
        if c == x.shape[1]:
            return x * inv
        return th.cat([x[:, :c, :, :] * inv, x[:, c:, :, :]], dim=1)
    ...
    def forward(self, x, temb=None, split=0):
        ...
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                x = self.conv_shortcut(self._apply_ode_shortcut_in_scale(x))
            else:
                x = self.nin_shortcut(self._apply_ode_shortcut_in_scale(x), split=split)
        out = x + h
        return out
