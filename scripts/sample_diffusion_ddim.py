...
try:
    from qdiff.recon_metrics import (
        ReconMetricsRegistry,
        load_layer_whitelist,
        DEFAULT_LAYERS_FILE,
    )
except ImportError:  # 可选：未安装/未提供 recon_metrics 时跳过曲线记录
    ReconMetricsRegistry = None  # type: ignore
    DEFAULT_LAYERS_FILE = ""

    def load_layer_whitelist(*_args, **_kwargs):
        return []
...
class Diffusion(object):
    ...
    def sample(self):
            ...
            if self.args.quant_mode == 'qdiff':
                ...
                if self.args.resume:
                    ...
                    def _get_recon_registry(stage: str):
                        if (
                            not recon_plot_dir
                            or stage not in recon_plot_stages
                            or ReconMetricsRegistry is None
                        ):
                            return None
                    ...
                    def recon_model(model, prefix=""):
                        ...
                        for name, module in model.named_children():
                            ...
                            if registry is not None:
                                call_kwargs["metrics_logger"] = registry.create_logger(full_name)
                                call_kwargs["recon_tag"] = full_name
        ...
        self.noise_corrector = None
        if getattr(self.args, "enable_learned_noise_corr", False):
            from noise_eps_corr.learned_noise_corrector import load_learned_corrector
            ckpt = (getattr(self.args, "learned_corr_ckpt", "") or "").strip()
            if not ckpt:
                raise ValueError("--enable_learned_noise_corr requires --learned_corr_ckpt")
            self.noise_corrector = load_learned_corrector(ckpt, self.device)
            self.noise_corrector.train_mode_off()
            if getattr(self.args, "learned_corr_t_cut", None) is not None:
                self.noise_corrector.meta.t_cut = int(self.args.learned_corr_t_cut)
            if getattr(self.args, "learned_corr_alpha", None) is not None:
                self.noise_corrector.meta.alpha = float(self.args.learned_corr_alpha)
            logger.info(
                "Learned ε-corrector ON ckpt=%s t_cut=%s alpha=%s",
                ckpt, self.noise_corrector.meta.t_cut, self.noise_corrector.meta.alpha,
            )
    ...
    def sample_image(self, x, model, last=True):
        ...
        if self.args.sample_type == "generalized":
            ...
            corr = getattr(self, "noise_corrector", None)

            def _model_for_dpm(x_in, t_in, **kwargs):
                eps = model(x_in, t_in, **kwargs)
                if corr is None:
                    return eps
                t_idx = t_in
                if torch.is_tensor(t_idx) and t_idx.ndim > 1:
                    t_idx = t_idx.reshape(t_idx.shape[0], -1)[:, 0]
                return corr.correct(eps, t_idx, None, xt=x_in)

            model_fn = model_wrapper(
                _model_for_dpm if corr is not None else model,
                noise_schedule,
                model_type="noise"
            )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="generalized",
        help="sampling approach (generalized | dpm_solver | ddpm_noisy)",
    )
    parser.add_argument(
        "--enable_learned_noise_corr",
        action="store_true",
        help="apply learned ε-corrector (supported for sample_type=dpm_solver)",
    )
    parser.add_argument(
        "--learned_corr_ckpt",
        type=str,
        default="",
        help="path to learned ε-corrector ckpt",
    )
    parser.add_argument("--learned_corr_t_cut", type=int, default=None)
    parser.add_argument("--learned_corr_alpha", type=float, default=None)
    parser.add_argument(
        "--skip_type",
        type=str,
        default="uniform",
